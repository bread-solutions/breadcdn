...
const dbClient = new MongoClient("mongodb+srv://" + config.database.user + ":" + config.database.password + "@" + config.database.host, {retryWrites: true, w: 'majority', serverApi: ServerApiVersion.v1});
 ...
 * @returns 
 */
export async function modifyUser(user: Partial<IUser>, updatedUser: Partial<IUser>): Promise<void> {
    const dbConnection = await dbClient.connect();
    const collection = dbConnection.db(config.database.name).collection('users');
 ...
 * @returns 
 */
export async function modfyUpload(upload: Partial<IUpload>, updatedUpload: Partial<IUpload>): Promise<void> {
    const dbConnection = await dbClient.connect();
    const collection = dbConnection.db(config.database.name).collection('uploads');
